...
export default function Dashboard() {
    ...
    return (
        <div className="min-h-screen bg-white flex flex-col font-sans">
            ...
            <div className="w-full bg-gradient-to-r from-[#064E3B] to-[#0a6c52] text-white py-8 px-6 sm:px-10 shadow-md rounded-none mb-8">
                <div className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                    <div>
                        ...
                        <p className="text-sm text-emerald-100">
                            You have <span className="font-bold text-white">{pendingCount}</span> pending task{pendingCount === 1 ? '' : 's'} today.
                        </p>
                    </div>
                    ...
                    <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 flex items-center gap-4 self-stretch sm:self-auto min-w-[180px] justify-between sm:justify-start">
                        ...
                        <div className="w-12 h-12 rounded-full border-4 border-emerald-300/40 border-t-white flex items-center justify-center font-bold text-xs">
            ...
            <main className="flex-1 w-full px-6 sm:px-10 pb-12">
                <div className="w-full bg-white rounded-3xl p-6 sm:p-8 shadow-lg border border-[#064E3B]/10 space-y-8">
                    {/* Stats summary pill items */}
                    <div className="grid grid-cols-3 gap-4">
                        <div 
                            onClick={() => setFilter('all')}
                            className={`p-4 sm:p-5 rounded-2xl border transition-all cursor-pointer text-center ${filter === 'all' ? 'bg-[#064E3B] text-white border-[#064E3B] shadow-md' : 'bg-slate-50 border-slate-200 text-slate-700 hover:border-[#064E3B]/40'}`}
                        >
                            <div className="text-2xl sm:text-3xl font-bold">{totalCount}</div>
                            <div className="text-xs font-bold uppercase tracking-wider opacity-80 mt-1">All Tasks</div>
                        </div>
                        <div 
                            onClick={() => setFilter('pending')}
                            className={`p-4 sm:p-5 rounded-2xl border transition-all cursor-pointer text-center ${filter === 'pending' ? 'bg-[#064E3B] text-white border-[#064E3B] shadow-md' : 'bg-amber-50/60 border-amber-200 text-amber-900 hover:border-amber-400'}`}
                        >
                            <div className="text-2xl sm:text-3xl font-bold">{pendingCount}</div>
                            <div className="text-xs font-bold uppercase tracking-wider opacity-80 mt-1">Pending</div>
                        </div>
                        <div 
                            onClick={() => setFilter('completed')}
                            className={`p-4 sm:p-5 rounded-2xl border transition-all cursor-pointer text-center ${filter === 'completed' ? 'bg-[#064E3B] text-white border-[#064E3B] shadow-md' : 'bg-emerald-50/60 border-emerald-200 text-emerald-900 hover:border-emerald-400'}`}
                        >
                            <div className="text-2xl sm:text-3xl font-bold">{completedCount}</div>
                            <div className="text-xs font-bold uppercase tracking-wider opacity-80 mt-1">Completed</div>
                        </div>
                    </div>

                    {/* Todo Add Form Section */}
                    <div className="pt-2">
                        <h2 className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-3">Add New Task</h2>
